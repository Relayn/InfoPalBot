...
from app.database.models import User, Subscription # Subscription используется в type hinting
...
async def send_weather_updates(bot: Bot):
    ...
    with get_session() as db_session: # Используем контекстный менеджер для сессии БД
        ...
        weather_subscriptions: List[Subscription] = get_active_subscriptions_by_info_type(
            session=db_session,
        ...
        for sub in weather_subscriptions:
            ...
            if not sub.details or not sub.user_id:
                logger.warning(f"Пропуск подписки ID {sub.id}: отсутствует город (details) или user_id.")
                continue
            ...
            if not user or not user.telegram_id:
                logger.warning(f"Не найден пользователь или telegram_id для подписки ID {sub.id} (user_id: {sub.user_id}).")
                continue

            telegram_id_to_send: int = user.telegram_id
            city_name: str = sub.details

            logger.info(f"Обработка подписки на погоду для пользователя {telegram_id_to_send}, город: {city_name}")
            ...
            if weather_data and not weather_data.get("error"):
                try:
                    ...
                    description: str = weather_data['weather'][0]['description'].capitalize()
                    ...
                    await bot.send_message(chat_id=telegram_id_to_send, text=message_text, parse_mode=ParseMode.HTML) # Явно указываем parse_mode
                    logger.info(f"Успешно отправлено уведомление о погоде пользователю {telegram_id_to_send} для города {city_name}.")
                except Exception as e:
                    logger.error(f"Ошибка при отправке уведомления о погоде пользователю {telegram_id_to_send} для города {city_name}: {e}", exc_info=True)
            elif weather_data and weather_data.get("error"):
                # Логируем ошибку, полученную от API-клиента
                logger.warning(f"Не удалось получить данные о погоде для рассылки (город: {city_name}): {weather_data.get('message')}")
            else:
                # Логируем случай, когда API-клиент вернул None или неожиданный результат
                logger.warning(f"Не удалось получить данные о погоде для рассылки (город: {city_name}), API вернул None или неожиданный результат.")
...
async def send_news_updates(bot: Bot):
    ...
    user_ids_subscribed_to_news: List[int] = list(set([sub.user_id for sub in all_news_subscriptions if sub.user_id]))
    ...
    articles_or_error: Optional[List[Dict[str, Any]]] | Dict[str, Any] = await get_top_headlines(country="ru", page_size=5)
    ...
    for i, article in enumerate(articles_or_error):
        ...
        source: str = html.escape(article.get('source', {}).get('name', 'Неизвестный источник'))
    ...
    with get_session() as db_session: # Отдельная сессия для получения User объектов
        for user_id in user_ids_subscribed_to_news:
            ...
            if user and user.telegram_id:
                try:
                    await bot.send_message(chat_id=user.telegram_id, text=news_message_text, disable_web_page_preview=True, parse_mode=ParseMode.HTML) # Явно указываем parse_mode
                    logger.info(f"Успешно отправлены новости пользователю {user.telegram_id}.")
                except Exception as e:
                    logger.error(f"Ошибка при отправке новостей пользователю {user.telegram_id}: {e}", exc_info=True)
            else:
...
async def send_events_updates(bot: Bot):
    ...
    with get_session() as db_session:
        ...
        events_by_location_slug: Dict[str, List[Dict[str, Any]]] = {} # Кэш для событий по городам
        users_for_location_slug: Dict[str, List[int]] = {} # telegram_id пользователей по городам
        ...
        for sub in event_subscriptions:
            ...
            user: Optional[User] = db_session.get(User, sub.user_id)
            if not user or not user.telegram_id:
                logger.warning(f"Не найден пользователь или telegram_id для подписки на события ID {sub.id} (user_id: {sub.user_id}).")
                continue

            if location_slug not in users_for_location_slug:
                users_for_location_slug[location_slug] = []
            if user.telegram_id not in users_for_location_slug[location_slug]: # Избегаем дублирования пользователя для одного города
        ...
        for location_slug, user_telegram_ids in users_for_location_slug.items():
            if not user_telegram_ids: continue # Пропускаем, если нет пользователей для этого города
            ...
            kudago_result: Optional[List[Dict[str, Any]]] | Dict[str, Any] = await get_kudago_events(location=location_slug, page_size=3)

            if isinstance(kudago_result, dict) and kudago_result.get("error"):
                logger.error(f"Ошибка API KudaGo при получении событий для '{location_slug}': {kudago_result.get('message')}")
            ...
            event_message_lines: List[str] = [f"<b>🎉 Ближайшие события в г. {html.escape(city_display_name)}:</b>"]
            for i, event_data in enumerate(kudago_result):
                ...
                description: str = html.unescape(description_raw.replace('<p>', '').replace('</p>', '').replace('<br>', '\n')).strip()
            ...
            for telegram_id in user_telegram_ids:
                try:
                    await bot.send_message(chat_id=telegram_id, text=event_message_text, disable_web_page_preview=True, parse_mode=ParseMode.HTML) # Явно указываем parse_mode
                    logger.info(f"Успешно отправлено уведомление о событиях для {location_slug} пользователю {telegram_id}.")
                except Exception as e:
                    logger.error(f"Ошибка при отправке уведомления о событиях для {location_slug} пользователю {telegram_id}: {e}", exc_info=True)
