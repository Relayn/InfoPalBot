# Файл: app/bot/handlers/subscription.py
...
from ..keyboards import get_frequency_keyboard
...
async def process_info_type_choice(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    with get_session() as db_session:
        ...
        if info_type == INFO_TYPE_NEWS:
            if get_subscription_by_user_and_type(db_session, user.id, info_type):
                await callback_query.message.edit_text("Вы уже подписаны на 'Новости'.")
                await state.clear()
                return
            await callback_query.message.edit_text("Вы выбрали 'Новости'.\nТеперь выберите частоту:",
...
async def process_city_for_events_subscription(message: types.Message, state: FSMContext):
    ...
    with get_session() as db_session:
        user = get_user_by_telegram_id(session=db_session, telegram_id=message.from_user.id)
        if get_subscription_by_user_and_type(db_session, user.id, INFO_TYPE_EVENTS, location_slug):
            await message.answer(f"Вы уже подписаны на события в городе '{html.escape(city_name)}'.")
            await state.clear()
            return
    ...
    with get_session() as db_session:
        ...
        for i, sub in enumerate(subscriptions):
            ...
            if sub.info_type == INFO_TYPE_WEATHER:
                ...
                details_str = "Новости (США)"
            elif sub.info_type == INFO_TYPE_EVENTS:
                city_name = next(
                    (name.capitalize() for name, slug in KUDAGO_LOCATION_SLUGS.items() if slug == sub.details),
                    sub.details)
                details_str = f"События: <b>{html.escape(city_name)}</b>"
            response_lines.append(f"{i + 1}. {details_str} ({schedule_str})")
        await message.answer("\n".join(response_lines))
        log_user_action(db_session, telegram_id, "/mysubscriptions", f"Displayed {len(subscriptions)} subs")


# --- ИЗМЕНЕНО: Отображение кнопок отписки ---
        ...
        for sub in subscriptions:
            ...
            if sub.info_type == INFO_TYPE_WEATHER:
                ...
                city_name = next(
                    (name.capitalize() for name, slug in KUDAGO_LOCATION_SLUGS.items() if slug == sub.details),
                    sub.details)
                details_str = f"События: {html.escape(city_name)}"
