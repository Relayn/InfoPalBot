...
async def test_process_mysubscriptions_command_no_user_sub(session_sub):
    ...
    with patch('app.bot.main.get_session', return_value=mock_generator), \
            patch('app.bot.main.get_user_by_telegram_id', return_value=None), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/mysubscriptions")
...
async def test_process_mysubscriptions_command_with_subscriptions_sub(db_user_sub, session_sub):
    ...
    mock_message.answer = AsyncMock()
    mock_message.from_user = MagicMock(spec=AiogramUser, id=db_user_sub.telegram_id)
    sub1 = DBSubscription(id=1, user_id=db_user_sub.id, info_type=INFO_TYPE_NEWS, frequency="daily", details=None,
                          status="active")
    sub2 = DBSubscription(id=2, user_id=db_user_sub.id, info_type=INFO_TYPE_WEATHER, frequency="daily",
                          details="Москва", status="active")
    sub3 = DBSubscription(id=3, user_id=db_user_sub.id, info_type=INFO_TYPE_EVENTS, frequency="daily", details="msk",
                          status="active")
    ...
    with patch('app.bot.main.get_session', return_value=mock_generator), \
            patch('app.bot.main.get_user_by_telegram_id', return_value=db_user_sub), \
            patch('app.bot.main.get_subscriptions_by_user_id',
                  return_value=mock_subs_list) as mock_get_user_subs_patched, \
            patch('app.bot.main.log_user_action') as mock_log_action:
        await process_mysubscriptions_command(mock_message)
...
async def test_process_unsubscribe_command_start_no_subscriptions_sub(db_user_sub, session_sub):
    ...
    with patch('app.bot.main.get_session', return_value=mock_generator), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/unsubscribe",
...
async def test_process_unsubscribe_command_start_with_subscriptions_sub(db_user_sub, session_sub):
    ...
    mock_message.from_user = MagicMock(spec=AiogramUser, id=db_user_sub.telegram_id)
    mock_state = AsyncMock(spec=FSMContext)
    sub1 = DBSubscription(id=10, user_id=db_user_sub.id, info_type=INFO_TYPE_WEATHER, frequency="daily", details="Сочи",
                          status="active")
    mock_subs_list = [sub1]
    mock_session_context_manager = MagicMock()
    mock_session_context_manager.__enter__.return_value = session_sub
    ...
    with patch('app.bot.main.get_session', return_value=mock_generator), \
        ...
        assert reply_markup.inline_keyboard[0][0].callback_data == f"unsubscribe_confirm:{sub1.id}"
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/unsubscribe",
                                                "Start unsubscribe process")
...
async def test_process_unsubscribe_confirm_success_sub(db_user_sub, session_sub):
    ...
    with patch('app.bot.main.get_session', return_value=mock_generator), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_callback_query.from_user.id, "unsubscribe_confirm_success",
...
async def test_process_unsubscribe_confirm_not_users_subscription_sub(db_user_sub, session_sub):
    ...
    other_user_sub_instance = DBSubscription(id=other_user_sub_id, user_id=999, info_type=INFO_TYPE_NEWS,
    ...
    with patch('app.bot.main.get_session', return_value=mock_generator), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_callback_query.from_user.id, "unsubscribe_error",
...
async def test_process_unsubscribe_action_cancel_sub():
    ...
    with patch('app.bot.main.log_user_action') as mock_log_action:
        ...
        mock_log_action.assert_called_once_with(ANY, mock_callback_query.from_user.id, "unsubscribe_action_cancel")