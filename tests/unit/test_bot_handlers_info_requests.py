...
async def test_process_weather_command_success():
    ...
    mock_weather_api_response = {
        ...
        "wind": {"speed": 3.0, "deg": 180}, "name": city_name
    }
    with patch('app.bot.main.get_weather_data', return_value=mock_weather_api_response), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/weather", f"город: {city_name}, успех")
...
async def test_process_weather_command_no_city():
    ...
    with patch('app.bot.main.log_user_action') as mock_log_action:
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/weather", "Город не указан")
...
async def test_process_weather_command_city_not_found():
    ...
    with patch('app.bot.main.get_weather_data', return_value=mock_weather_api_error_response), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/weather", f"город: {city_name}, ошибка API: city not found")
...
async def test_process_weather_command_api_key_error():
    ...
    with patch('app.bot.main.get_weather_data', return_value=mock_weather_api_error_response), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/weather", f"город: {city_name}, ошибка API: Invalid API key")
...
async def test_process_weather_command_api_other_error():
    ...
    with patch('app.bot.main.get_weather_data', return_value=mock_weather_api_error_response), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/weather", f"город: {city_name}, ошибка API: Some other API error")
...
async def test_process_weather_command_parsing_key_error():
    ...
    mock_message.from_user = MagicMock(spec=AiogramUser, id=123, full_name="Tester")
    mock_command = MagicMock(spec=CommandObject, args=city_name)
    malformed_weather_api_response = {"weather": [{"description": "ясно"}], "wind": {"speed": 3.0, "deg": 180}, "name": city_name}
    with patch('app.bot.main.get_weather_data', return_value=malformed_weather_api_response), \
         patch('app.bot.main.log_user_action') as mock_log_action:
        await process_weather_command(mock_message, mock_command)
        mock_message.reply.assert_any_call(f"Запрашиваю погоду для города <b>{html.escape(city_name)}</b>...")
        mock_message.answer.assert_called_once_with("Не удалось обработать данные о погоде...") # Сокращенное сообщение
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/weather", f"город: {city_name}, ошибка парсинга: 'main'") # Исправлен details
...
async def test_process_news_command_success():
    ...
    with patch('app.bot.main.get_top_headlines', return_value=mock_articles), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/news", "success")
...
async def test_process_news_command_no_articles():
    ...
    with patch('app.bot.main.get_top_headlines', return_value=[]), \
        ...
        mock_message.reply.assert_any_call("На данный момент нет главных новостей для отображения.") # Полный текст
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/news", "no_articles_found")
...
async def test_process_news_command_api_error():
    ...
    with patch('app.bot.main.get_top_headlines', return_value=mock_api_error_response), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/news", f"api_error: {error_message_from_api[:100]}")
...
async def test_process_news_command_unexpected_return():
    ...
    with patch('app.bot.main.get_top_headlines', return_value=None), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/news", "unexpected_api_response")
...
async def test_process_events_command_success():
    ...
    with patch('app.bot.main.get_kudago_events', return_value=mock_events), \
        ...
        expected_text = "\n\n".join(expected_lines)
        mock_message.answer.assert_called_once_with(expected_text, disable_web_page_preview=True)
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/events", f"город: {city_arg}, успех")
...
async def test_process_events_command_no_city_arg():
    ...
    with patch('app.bot.main.log_user_action') as mock_log_action:
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/events", "Город не указан")
...
async def test_process_events_command_unknown_city():
    ...
    with patch('app.bot.main.log_user_action') as mock_log_action:
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/events", f"город: {city_arg}, город не поддерживается")
...
async def test_process_events_command_no_events_found():
    ...
    with patch('app.bot.main.get_kudago_events', return_value=[]), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/events", f"город: {city_arg}, не найдено")
...
async def test_process_events_command_api_error():
    ...
    with patch('app.bot.main.get_kudago_events', return_value=mock_api_error_response), \
        ...
        mock_log_action.assert_called_once_with(ANY, mock_message.from_user.id, "/events", f"город: {city_arg}, ошибка API: {error_message_from_api[:70]}") # Исправлено 'api_error' на 'ошибка API'