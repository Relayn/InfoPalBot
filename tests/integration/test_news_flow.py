...
async def test_news_command_successful_flow(integration_session: Session):
    ...
    mock_message.from_user = MagicMock(
        spec=AiogramUser, id=telegram_user_id, full_name="News User"
    )
    ...
    mock_api_response_data = {
        ...
        "articles": [
            {
                "title": "Новость 1 <script>alert(0)</script>",
    ...
    mock_news_client_instance.get.assert_called_once_with(
        expected_news_api_url, params=expected_news_api_params
    )

    expected_lines = ["<b>📰 Последние главные новости (Россия):</b>"]
    title1_escaped = html.escape("Новость 1 <script>alert(0)</script>")
    expected_lines.append(
        f"1. <a href='http://example.com/news1'>{title1_escaped}</a> (Источник 1)"
    )
    expected_lines.append(
        "2. <a href='http://example.com/news2'>Новость 2</a> (Источник 2)"
    )
    expected_text = "\n".join(expected_lines)
...
async def test_news_command_api_error_flow(integration_session: Session):
    ...
    mock_message = AsyncMock(spec=Message)
    mock_message.from_user = MagicMock(
        spec=AiogramUser, id=telegram_user_id, full_name="Error News User"
    )
    mock_message.chat = MagicMock(spec=Chat, id=telegram_user_id)
    mock_message.reply = AsyncMock()
    mock_message.answer = AsyncMock()
    ...
    mock_httpx_response.status_code = (
        200  # NewsAPI может вернуть 200 OK, но с ошибкой в JSON
    )
...
async def test_news_command_no_articles_flow(integration_session: Session):
    ...
    mock_message = AsyncMock(spec=Message)
    mock_message.from_user = MagicMock(
        spec=AiogramUser, id=telegram_user_id, full_name="No News User"
    )
    mock_message.chat = MagicMock(spec=Chat, id=telegram_user_id)
    mock_message.reply = AsyncMock()
    mock_message.answer = AsyncMock()
